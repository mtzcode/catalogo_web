    ...
    return (
      <div className="max-w-2xl mx-auto">
        ...
        <div className="rounded-xl border bg-white p-6 flex flex-col items-center justify-center min-h-[300px]">
          <div className="text-center text-tertiary mb-4">
            Seu carrinho está vazio
          </div>
          <Link
            href="/"
            className="inline-flex items-center justify-center px-4 py-2 rounded-md bg-secondary text-white hover:opacity-90 focus-visible:outline-none focus-visible:ring-1 focus-visible:ring-gray-300"
          >
            Ver produtos
          </Link>
        </div>
      ...
      <div className="rounded-xl border bg-white p-4">
        <div className="divide-y">
          {items.map(({ product, qty }) => {
            const isPromo =
              !!product.promocaoAtiva && !!product.precoPromocional;
            const basePrice = isPromo
              ? (product.precoPromocional as number)
            ...
            return (
              ...
              >
                <div className="w-20 h-20 sm:w-16 sm:h-16 aspect-square rounded-md bg-gray-100 flex items-center justify-center overflow-hidden">
                  {product.imagemUrl || product.codigoBarras ? (
                    errorIds.has(String(product.id)) ? (
                      ...
                      <Image
                        src={
                          product.imagemUrl ??
                          cloudinaryUrl(product.codigoBarras!, {
                            withExt: true,
                            transformations: [
                              "f_auto",
                              "q_auto:good",
                ...
                <div className="flex-1">
                  ...
                  <div className="text-xs text-tertiary">Qtd: {formatQtyGrams(product, qty)}</div>
                  <div className="text-[11px] text-tertiary">Preço unitário: {formatBRL(unitPrice)}{isWeight ? ' /100g' : ''}</div>
                </div>
                <div className="flex items-center gap-2 flex-wrap sm:flex-nowrap">
                  <button
                    onClick={() => dec(product.id)}
                    className="h-8 w-8 rounded-md border flex items-center justify-center focus-visible:outline-none focus-visible:ring-1 focus-visible:ring-gray-300"
                  >
                    -
                  </button>
                  <div className="w-16 text-center text-sm">{isWeight ? formatQtyGrams(product, qty) : qty}</div>
                  <button
                ...
                <div className="w-full sm:w-24 sm:text-right text-sm font-semibold mt-2 sm:mt-0">
                  {formatBRL(lineTotal)}
                </div>
                <button
        ...
        <div className="flex justify-between items-center mt-4">
          <button
            onClick={clear}
            className="text-sm text-red-600 hover:text-red-700 focus-visible:outline-none focus-visible:ring-1 focus-visible:ring-gray-300"
          >
            Limpar carrinho
          </button>
        ...
        <div className="mt-4">
          <Link
            href="/checkout"
            className="w-full inline-flex items-center justify-center h-10 rounded-md bg-secondary text-white text-sm font-medium hover:opacity-90 focus-visible:outline-none focus-visible:ring-1 focus-visible:ring-gray-300"
          >
            Continuar
          </Link>
        </div>
